...
export default function IntroModal({ onComplete }) {
  ...
  return (
    <div className={`fixed inset-0 bg-gradient-to-br from-purple-900 via-pink-800 to-orange-700 flex items-center justify-center z-50 p-4 transition-all duration-700 ${
      isExiting ? 'animate-out fade-out' : 'animate-in fade-in'
    }`}>
      {/* Branding watermark */}
      <div className="absolute inset-0 flex items-center justify-center opacity-5 pointer-events-none">
        <div className="text-[200px] font-bold text-white tracking-widest">MONO</div>
      </div>
      ...
      <div 
        ref={stepRef}
        className={`relative bg-white/10 backdrop-blur-xl rounded-3xl max-w-lg w-full p-10 shadow-2xl border border-white/20 transition-all duration-700 ${
        ...
        <div className="text-center mb-8">
          <div className={`w-24 h-24 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-6 transition-all duration-700 ${
            isTransitioning ? 'scale-0' : 'scale-100'
          }`}>
            <Icon className={`w-12 h-12 text-white transition-all duration-700 ${
              isTransitioning ? 'scale-0' : 'scale-100'
            }`} />
          </div>
          
          {/* Title */}
          <h2 className={`text-4xl font-bold text-white mb-2 transition-all duration-700 ${
          ...
          {/* Subtitle */}
          {step.subtitle && (
            <p className={`text-lg text-white/80 mb-4 tracking-widest uppercase transition-all duration-700 delay-100 ${
              isTransitioning ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'
            }`}>
              {step.subtitle}
            </p>
          ...
          {/* Description */}
          <p className={`text-white/70 text-lg leading-relaxed transition-all duration-700 delay-200 ${
            isTransitioning ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'
          }`}>
            {step.description}
          </p>
        ...
        <div className={`mt-8 text-center transition-all duration-700 delay-500 ${
          ...
          <p className="text-sm text-white/50 tracking-widest uppercase">MONO BOOTH PH</p>
          <p className="text-xs text-white/30 mt-1">📍 Kabankalan City & Beyond</p>
        </div>
      </div>
    </div>
  );
